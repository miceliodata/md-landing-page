...
const size = props.size || Math.min(window.innerWidth, window.innerHeight) * 0.75
...
const generate3DWireframe = (rotationY: number = 0) => {
  ...
  for (let lat = -80; lat <= 80; lat += 20) {
    ...
    for (let lon = 0; lon <= 360; lon += 5) {
      ...
      if (z > -radius * 0.3) { // Only show front hemisphere
...
const generateContinents = (rotationY: number = 0) => {
  ...
  const asiaPoints = [
    [60, 20], [80, 30], [120, 35], [140, 40], [120, 50], [100, 60], [80, 45], [60, 35]
  ]

  // Europe outline
  const europePoints = [
    [-10, 35], [20, 45], [30, 60], [20, 65], [0, 55], [-15, 50]
  ]
...
const handleScroll = () => {
  ...
  if (globeRef.value) {
    const connectionElements = globeRef.value.querySelectorAll('.connection-line, .connection-dot')
    ...
    >
      ...
      <g class="globe-wireframe">
        ...
        <g class="latitude-lines">
          <template v-for="(line, index) in wireframeLines.filter(l => l.type === 'latitude')" :key="`lat-${index}`">
            <path
        ...
        <g class="longitude-lines">
          <template v-for="(line, index) in wireframeLines.filter(l => l.type === 'longitude')" :key="`lng-${index}`">
            <path
              :d="`M ${line.points.map(p => `${p.x},${p.y}`).join(' L ')}`"
              fill="none"
      ...
      <!-- Continent outlines -->
      <g class="continents">
        <template v-for="(continent, index) in continents" :key="`continent-${index}`">
          <path
            v-if="continent.points.length > 2"
            :d="`M ${continent.points.map(p => `${p.x},${p.y}`).join(' L ')} Z`"
      ...
      <g class="connections">
        <!-- Connection lines -->
        <template v-for="(country, index) in connectionPoints.asia" :key="`connection-${index}`">
          <path
            v-if="country.visible && connectionPoints.spain.visible"
            :d="`M ${country.x} ${country.y} Q ${centerX + (country.x - connectionPoints.spain.x) * 0.3} ${centerY - 80} ${connectionPoints.spain.x} ${connectionPoints.spain.y}`"
            fill="none"
        ...
        <!-- Asian country dots -->
        <template v-for="(country, index) in connectionPoints.asia" :key="`asia-dot-${index}`">
          <circle
