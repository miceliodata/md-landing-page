...
const createWireframeEarth = () => {
  ...
  const silkRoadGeometry = new THREE.BufferGeometry().setFromPoints(silkRoadPath)
...
const animate = () => {
  ...
  if (earth) {
    ...
    const silkRoadLine = earth.getObjectByName('silkRoad')
    const silkRoadGlow = earth.getObjectByName('silkRoadGlow')
    ...
    if (silkRoadLine && silkRoadLine.material) {
      ...
      if (scrollProgress > 0.2) {
        ...
        silkRoadLine.material.opacity = lineProgress * 0.9

        // Animate glow effect
        if (silkRoadGlow && silkRoadGlow.material) {
          silkRoadGlow.material.opacity = lineProgress * 0.4
        }
      } else {
        silkRoadLine.material.opacity = 0
        if (silkRoadGlow && silkRoadGlow.material) {
          silkRoadGlow.material.opacity = 0
        }
      }
    }
  }
