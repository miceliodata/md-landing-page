<script setup lang="ts">
</script>

<template>
  <header class="bg-white border-b border-gray-200 px-6 py-4 flex justify-between items-center sticky top-0 z-50">
    <div class="text-xl font-bold">MICELIO DATA</div>
    <nav>
      <ul class="flex space-x-6 text-sm font-medium">
        <li><a href="#" class="hover:text-blue-600">Platform</a></li>
        <li><a href="#" class="hover:text-blue-600">Suppliers</a></li>
        <li><a href="#" class="hover:text-blue-600">Brands</a></li>
        <li><a href="#" class="hover:text-blue-600">Partners</a></li>
      </ul>
    </nav>
    <a href="#" class="bg-black text-white px-4 py-2 rounded hover:bg-gray-800 text-sm font-semibold">
      Request a demo
    </a>
  </header>
</template>